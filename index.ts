...
const handler = (content: Nullable<WsContent> | undefined | null) => (_: Request, res: Response) => {
...
(ast.filter(it => (it instanceof WsEndpoint)) as WsEndpoint[])
    .forEach((it: WsEndpoint) => {
        const content = it.responses.filter(it => it?.content?.type === "application/json")[0]?.content
