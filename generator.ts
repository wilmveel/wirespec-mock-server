...
export const generator = (ast: WsNode[]) => {
  ...
  const generateRandomNode = (
    ...
    if (node instanceof WsType)
      return node?.shape.value.reduce(
        (acc, cur) => ({
          ...acc,
    ...
    if (node instanceof WsEndpoint)
      throw new Error(`Cannot generate random WsEndpoint`);
    throw new Error(`Cannot generate random node`);
  };

  const generateRandomReference = (
    reference: WsReference,
  ):
    | (Record<string, string> | string | undefined)[]
    | (Record<string, string> | string | undefined) => {
